...
/*
 * Xiaomi's downstream driver used an Android-only three-state speaker-ID
 * helper.  Equuleus does not describe that GPIO, so use the stock generic
 * firmware selected by VENDOR_ID_NONE.
 */
enum tas2557_speaker_vendor {
	VENDOR_ID_NONE = 0,
	VENDOR_ID_UNKNOWN,
	VENDOR_ID_AAC,
	VENDOR_ID_GOER,

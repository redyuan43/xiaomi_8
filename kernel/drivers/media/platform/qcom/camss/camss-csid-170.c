...
static void csid_configure_stream(struct csid_device *csid, u8 enable)
{
	...
	if (enable) {
		...
		/* For a raw RDI path, retain the packet payload packing. */
...
static irqreturn_t csid_isr(int irq, void *dev)
{
	...
	top_status = readl_relaxed(csid->base + CSID_TOP_IRQ_STATUS);
	writel_relaxed(top_status, csid->base + CSID_TOP_IRQ_CLEAR);
	reset_done = top_status & BIT(TOP_IRQ_STATUS_RESET_DONE);

	rx_status = readl_relaxed(csid->base + CSID_CSI2_RX_IRQ_STATUS);
	writel_relaxed(rx_status, csid->base + CSID_CSI2_RX_IRQ_CLEAR);

	rdi_status = readl_relaxed(csid->base + CSID_CSI2_RDIN_IRQ_STATUS(0));
	writel_relaxed(rdi_status, csid->base + CSID_CSI2_RDIN_IRQ_CLEAR(0));

	writel_relaxed(1 << IRQ_CMD_CLEAR, csid->base + CSID_IRQ_CMD);

	if (rx_status || rdi_status ||
	    (top_status & ~BIT(TOP_IRQ_STATUS_RESET_DONE)))
		dev_warn_ratelimited(csid->camss->dev,
				     "CSID%u IRQ top=%#x rx=%#x rdi=%#x\n",
				     csid->id, top_status, rx_status, rdi_status);

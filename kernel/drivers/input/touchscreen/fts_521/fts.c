...
#ifdef CONFIG_TOUCHSCREEN_ST_FTS_V521_DRM_NOTIFIER
...
/*
 * Android's downstream backlight notifier and factory proc endpoints are not
 * available in the 5.12 port.  They are not needed for basic I2C touch input.
 */
#ifndef BACKLIGHT_UPDATED
#define BACKLIGHT_UPDATED 0
#define BACKLIGHT_OFF 1
#define BACKLIGHT_ON 0
#endif
#define backlight_register_notifier(_nb) 0
#define backlight_unregister_notifier(_nb) do { } while (0)
#define proc_create(...) NULL
...
bool fts_fod_status;
#endif
/* Android's display low-power manager is not present in the mainline port. */
				  ...
				  const char *buf, size_t count)
{
	...
	for (n = 0; n < (count + 1) / 3; n++) {
		sscanf(p, "%02hhX ", &typeOfCommand[n]);
		p += 3;
...
static int fts_probe(struct spi_device *client)
{
	...
	/* Keep the controller's known-good Android firmware during porting. */
	pr_info("Skip FTS firmware update in temporary Linux boot\n");
	error = fts_init_sensing(info);
	if (error < OK) {
		pr_err("Cannot initialize FTS sensing ERROR %08X\n",
			error);
		error = -ENODEV;
		goto ProbeErrorExit_7;
	}
